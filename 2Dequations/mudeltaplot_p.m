function V =mudeltaplot_p(p,a,b,c,A,B,Nx,Ny,tol,u,Delta)
...
% Sample usage:
% V=mudeltaplot_p(3,1,-1.7,0,20,20,500,500,1e-10,0,.4:.01:1.8)
% V=mudeltaplot_p(3,1,0,1.35,20,20,500,500,1e-10,0,.4:.01:1.8)
% V=mudeltaplot_p(3,0,1,0,10,10,500,500,1e-10,0,.4:.01:1.8)
...
delta
newE=gradsolver_p(p,a,b,c,A,B,Nx,Ny,tol,u,delta,0);
mu=mean(newE(end-n:end)./newE(end-n-1:end-1));
Mu1=[Mu1 mu];

newE=rgradsolver_p(p,a,b,c,A,B,Nx,Ny,tol,u,delta,0);
mu=mean(newE(end-n:end)./newE(end-n-1:end-1));
Mu2=[Mu2 mu];

newE=rnewtsolver_p(p,a,b,c,A,B,Nx,Ny,tol,u,delta,1,0);
mu=mean(newE(end-n:end)./newE(end-n-1:end-1));
Mu3=[Mu3 mu];

newE=rnewtsolver_p(p,a,b,c,A,B,Nx,Ny,tol,u,delta,2,0);
mu=mean(newE(end-n:end)./newE(end-n-1:end-1));
Mu4=[Mu4 mu];
