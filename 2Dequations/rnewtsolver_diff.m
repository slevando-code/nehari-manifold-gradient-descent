function E=rnewtsolver_diff(p,q,a,b,c,A,B,Nx,Ny,tol,u,delta,n,plot_final)


% Numerically approximate solutions of
% u_xxxx+2u_xxyy+u_yyyy-b(u_xx+u_yy)+c^2u_xx+au=|u|^(p-1)*u
% Sample usage:
% E=rnewtsolver_diff(3,6,0,1,0,10,10,500,500,1e-10,0,1,2,1)
% E=rnewtsolver_diff(3,6,1,-1.7,0,20,20,1000,1000,1e-10,0,1,2,1)
% E=rnewtsolver_diff(3,6,1,0,1.35,20,20,1000,1000,1e-10,0,1,2,1)
...
while (H2err>tol)
    ...
    gradP=real(2*u-ifft2(fft2(f+u.*df)./m));
    ...
    w0 = gradS-real(sum(sum(m.*fft2(gradS).*conj(fft2(gradP))))/sum(sum(m.*fft2(gradP).*conj(fft2(gradP)))))*gradP;
    w=w0;

    for i=1:n
        J=ifft2(fft2(df.*w)./m);
        J=J-real(sum(sum(m.*fft2(J).*conj(fft2(gradP))))/sum(sum(m.*fft2(gradP).*conj(fft2(gradP)))))*gradP;
